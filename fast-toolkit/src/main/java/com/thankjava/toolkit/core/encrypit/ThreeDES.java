...
public final class ThreeDES {
    ...
    public static String encryptStr(String key, String encryptStr) {
        ...
        String tempKey = diverse(KEY, key);

        String result = new String(hex(encryptStr.getBytes()));
        result += "80";
        while (result.length() % 16 != 0) {
            result += "0";
        }
        return encrypt(tempKey, result);
    }
    ...
    public static String decodeStr(String key, String decodeStr) {
        ...
        key = key.substring(0, 16);
        String tempKey = diverse(KEY, key);
        String result = deEncrypt(tempKey, decodeStr);
        if (result.length() % 16 == 0) {
            int dex = result.lastIndexOf("80");
            String cutStr = result.substring(0, dex);
    ...
    private static String deEncrypt(String key, String data) {
